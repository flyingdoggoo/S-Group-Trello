...
export class ProjectsController {
	...
	async getProjectMembers(req: Request, res: Response, next: NextFunction) {
		try {
			const { id } = req.params;
			const userId = req.user?.id as string;
			const result = await this.projectsService.getProjectMembers(id, userId);
	...
	async changeRoleOfMemberProject(req: Request, res: Response, next: NextFunction) {
		try {
			const { id } = req.params;
			const { userId, newRoleId } = req.body;

			const result = await this.projectsService.changeRoleOfMemberProject(
				id,
				userId,
				newRoleId,
			);
	...
	async removeMember(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await this.projectsService.removeMember(id, userId);
